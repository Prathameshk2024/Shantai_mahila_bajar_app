...
adminRouter.get('/stats', (_req, res) => {
  ...
  const stats: AdminStats = {
    ...
    funnel: [
      ...
      { mr: '50 रुपये भरले', en: 'Paid', v: db.payments.length },
...
adminRouter.get('/payments', (req, res) => {
  ...
  const vendorTotals = [...new Map(
    approved.map((p) => [p.sellerId, {
      sellerId: p.sellerId,
      sellerName: p.sellerName,
      womenBizId: p.womenBizId,
      upiId: db.sellers.find((s) => s.id === p.sellerId)?.upiId,
      totalReceived: approved.filter((x) => x.sellerId === p.sellerId).reduce((n, x) => n + x.amount, 0),
    }]),
  ).values()]
  ...
  res.json({
    payments: list,
    income: {
      totalReceived: approved.reduce((n, p) => n + p.amount, 0),
...
adminRouter.post('/payments/:id/approve', (req, res) => {
  ...
  if (payment.status !== 'PENDING') {
    res.status(409).json({ error: `Already ${payment.status}` })
    return
  }
  ...
  if (seller) {
    ...
    if (purpose === 'REGISTRATION') seller.status = 'ACTIVE'
  }
  save()
...
adminRouter.post('/payments/:id/reject', (req, res) => {
  ...
  payment.verifiedAt = new Date().toISOString()
  payment.verifiedBy = req.auth!.userId
  payment.purpose = payment.purpose ?? 'REGISTRATION'

  const seller = db.sellers.find((s) => s.id === payment.sellerId)
  if (seller && payment.purpose === 'REGISTRATION') seller.status = 'PAYMENT_REJECTED'
  save()
  res.json({ payment })
})
...
/**
 * Admin can approve or reject a product. There is intentionally no admin
 * delete endpoint. Rejected products remain visible to the seller for 48
 * hours, then the server automatically archives them.
 */
adminRouter.post('/products/:id/moderate', (req, res) => {
  ...
  if (approve && product.isFood && !product.fssai) {
    res.status(409).json({
      error: 'Food listing has no FSSAI number',
      messageMr: 'FSSAI क्रमांकाशिवाय खाद्यपदार्थ प्रकाशित करता येणार नाही',
    })
    return
  }
  ...
  if (approve) {
    ...
    product.autoDeleteAt = undefined
  } else {
    const rejectedAt = new Date()
    product.status = 'REJECTED'
    product.rejectReason = String(req.body?.reason ?? '').trim() || 'Product did not meet the platform requirements.'
    product.rejectedAt = rejectedAt.toISOString()
    product.autoDeleteAt = new Date(rejectedAt.getTime() + 48 * 3_600_000).toISOString()
  }

  save()
  res.json({
    product,
    notification: approve ? null : {
      type: 'PRODUCT_REJECTED',
      sellerId: product.sellerId,
      message: `Your product was rejected. It will be removed automatically after 48 hours (${product.autoDeleteAt}).`,
    },
  })
})
...
adminRouter.get('/orders', (req, res) => {
  ...
  res.json({
    orders: list.sort((a, b) => b.placedAt.localeCompare(a.placedAt)).map((o) => ({
      ...o,
...
adminRouter.get('/sellers', (_req, res) => {
  ...
  res.json({
    sellers: db.sellers.map((s) => {
      const products = db.products.filter((p) => p.sellerId === s.id && p.status !== 'ARCHIVED')
...
adminRouter.post('/sellers/:id/block', (req, res) => {
  ...
  seller.status = req.body?.blocked ? 'BLOCKED' : 'ACTIVE'
  save()
  res.json({ seller })
})
...
adminRouter.get('/impact', (_req, res) => {
  ...
  res.json({
    ...
    readiness: db.sellers.map((s) => ({ womenBizId: s.womenBizId, village: s.village, score: s.readinessScore, band: s.readinessBand })),
  })
})
