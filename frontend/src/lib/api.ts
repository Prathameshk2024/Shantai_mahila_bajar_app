import type {
  Address, AdminPaymentAccount, Category, DigitalProfile, Feedback, Order, Product,
...
export const api = {
  sendOtp: (phone: string) => post<{ sent: boolean; demoCode?: string; cooldownMs?: number }>('/auth/otp/send', { phone }),
  verifyOtp: (phone: string, code: string, role: 'seller' | 'customer') =>
    post<{ registered: boolean; session?: Session; phone?: string }>('/auth/otp/verify', { phone, code, role }),

  registerSeller: (body: SellerRegistration) => post<{ seller: Seller; session: Session }>('/sellers/register', body),
  me: () => get<{ seller: Seller; slots: SlotInfo }>('/sellers/me'),
  updateMe: (patchBody: Partial<Seller>) => patch<{ seller: Seller }>('/sellers/me', patchBody),
  ...
  submitPayment: (utr: string, payerUpi?: string) => post<{ payment: SubscriptionPayment; status: Seller['status'] }>('/sellers/me/subscription/payment', { utr, payerUpi }),

  myProducts: () => get<{ products: Product[]; slots: SlotInfo }>('/products/mine'),
  createProduct: (body: Partial<Product> & { asDraft?: boolean }) => post<{ product: Product }>('/products', body),
  updateProduct: (id: string, body: Partial<Product>) => patch<{ product: Product }>(`/products/${id}`, body),
  ...
  catalog: (params: { categoryId?: string; q?: string; pincode?: string } = {}) => {
    ...
    const s = qs.toString()
    return get<{ products: (Product & { seller?: Partial<Seller> })[] }>(`/catalog/products${s ? `?${s}` : ''}`)
  },
  ...
  advanceOrder: (id: string, to: Order['status'], extra?: { otp?: string; reason?: string }) => post<{ order: Order }>(`/orders/${id}/advance`, { to, ...extra }),
  ...
  submitFeedback: (orderId: string, rating: number, comment?: string) =>
    post<{ feedback: Feedback }>('/feedback', { orderId, rating, comment }),
  myFeedback: () => get<{ feedback: Feedback[] }>('/feedback/mine'),
