...
export async function verifyWidgetOtp(code: string): Promise<string> {
  ...
  const data = await call('verifyOtp', code).catch((err: Error) => {
    // MSG91's own words for the same thing, in case its session went before
    // ours did - a resend that failed, or a widget reinitialised under us.
    throw /reqid/i.test(err.message) ? new Error(WIDGET_SESSION_LOST) : err
  })
